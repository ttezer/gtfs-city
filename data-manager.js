window.DataManager = (function () {
  ...
  function applyGtfsRuntimeData(runtimeData) {
    ...
    ctx.AppState.trips = runtimeData.nTRIPS;
    ctx.AppState.shapes = runtimeData.nSHAPES;
    ctx.AppState.stops = runtimeData.nSTOPS;
    ctx.AppState.stopInfo = runtimeData.nSTOP_INFO;
    ctx.AppState.stopDeps = runtimeData.nSTOP_DEPS;
    ctx.AppState.hourlyCounts = runtimeData.nHOURLY_COUNTS;
    ctx.AppState.hourlyHeat = runtimeData.nHOURLY_HEAT;
    ...
    ctx.AppState.stopNames = Object.entries(ctx.AppState.stopInfo).map(([sid, info]) => [
    ...
    ctx.setStopNames(ctx.AppState.stopNames);
    ...
    ctx.updateDensityGrid();
    if (!ctx.AppState.baseRuntimeData && runtimeData.nTRIPS.length > 0) {
      ctx.AppState.baseRuntimeData = ctx.captureRuntimeDataSnapshot();
    }
  ...
  async function confirmGtfsImport(options = {}) {
    ...
    ctx.uploadedGtfsCities.clear();
    ctx.hiddenCities.clear();
    ...
    if (!ok) {
      window.AppManager?.setLandingUploadState?.({ loading: false });
      ctx.uploadedGtfsCities.delete(meta.id);
      const idx = ctx.CITIES.findIndex((city) => city.id === meta.id);
      if (idx >= 0) ctx.CITIES.splice(idx, 1);
      ctx.buildCityList();
