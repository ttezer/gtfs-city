window.PlannerManager = (function () {
  ...
  function dijkstra(from, to) {
    ...
    if (!adj?.[from] && from !== to) return null;
    const dist = { [from]: 0 };
    const prev = {};
    ...
    while (heap.size) {
      ...
      visited.add(stopId);
      if (stopId === to) break;
      for (const [nextId, seconds, line] of (adj[stopId] || [])) {
  ...
  function calcReachabilityFromStop(startSid, maxSecs = 3600, options = {}) {
    ...
    if (!Object.keys(adj).length || !Object.keys(stopInfo).length) return [];
  ...
  function calcIsochronFromStop(startSid, maxSecs = 3600) {
    const ctx = getCtx();
    if (!Object.keys(getStopInfo(ctx)).length) return [];
  ...
  function findNearestStopToCoord(lon, lat) {
    const ctx = getCtx();
    const stopInfo = getStopInfo(ctx);
    if (!Object.keys(stopInfo).length) return null;
    let bestStopId = null;
    let bestDistance = Infinity;
    for (const [stopId, info] of Object.entries(stopInfo)) {
      const dx = info[0] - lon;
      const dy = info[1] - lat;
  ...
  function updateIsochronPanel(stopId, data) {
    const ctx = getCtx();
    const info = getStopInfo(ctx)?.[stopId];
  ...
  function showSelectedRoute() {
    const ctx = getCtx();
    const stopInfo = getStopInfo(ctx);
    if (!stopInfo?.[fromStopId] || !stopInfo?.[toStopId]) {
      showPlannerMessage(
