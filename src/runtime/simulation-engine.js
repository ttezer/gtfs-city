window.SimulationEngine = (function () {
  ...
  function getMap(ctx) {
    return ctx?.getMap ? ctx.getMap() : ctx?.mapgl;
  }
  ...
  function calcHeadwayPairs(time) {
    ...
    const result = window.AnalyticsUtils.calcHeadwayPairs({
      trips: ctx.focusedRoute ? getTrips(ctx).filter((trip) => trip?.s === ctx.focusedRoute) : getTrips(ctx),
      time,
  ...
  function animate(ts) {
    ...
    if (animFrame % 3 === 0) {
      ...
      const activeStopData = ctx.getActiveStopData ? ctx.getActiveStopData() : ctx.activeStopData;
      if (selectedTripIdx !== null) ctx.updateVehiclePanel();
