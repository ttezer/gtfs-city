(function attachStopConnectivityUtils(globalScope) {
  ...
  function buildGtfsHash(ctx) {
    ...
    const lastTrip = ctx.TRIPS?.[ctx.TRIPS.length - 1];
    return hashString([
      ctx.feed_id || ctx.activeCityId || 'feed',
      ctx.TRIPS?.length || 0,
      Object.keys(ctx.STOP_INFO || {}).length,
      firstTrip?.s || '',
      lastTrip?.s || '',
    ].join('|'));
  }
  ...
  function buildSnapshotFileName(ctx, options = {}) {
    ...
    const feedId = String(ctx?.feed_id || ctx?.activeCityId || 'active_feed')
  ...
  function getWindowDepartures(stopId, ctx, profile) {
    const deps = ctx.STOP_DEPS?.[stopId] || [];
  ...
  function getReachability(stopId, ctx, params) {
    ...
    while (queue.size) {
      ...
      for (const [nextId, seconds, line] of (ctx.ADJ[current.stopId] || [])) {
    ...
    return Array.from(dist.entries()).map(([reachableStopId, seconds]) => ({
      ...
      seconds,
      info: ctx.STOP_INFO[reachableStopId],
  ...
  function getStopConnectivityRecord(stopId, ctx, options = {}) {
    if (!stopId || !ctx?.STOP_DEPS?.[stopId]?.length) return null;
    const profile = buildProfile(options.profile);
    const params = buildParams(options.params);
    const record = buildStopRecord(stopId, ctx, profile, params);
    if (ctx?.AppState) {
      if (!ctx.AppState.stopConnectivityScores) {
        ctx.AppState.stopConnectivityScores = {
          meta: {
            feed_id: ctx.feed_id || ctx.activeCityId || 'active_feed',
      ...
      ctx.AppState.stopConnectivityScores.stops[stopId] = record;
    }
    return record;
  }

  function ensureStopConnectivityRecord(stopId, ctx, options = {}) {
    if (!stopId || !ctx?.STOP_DEPS?.[stopId]?.length) return;
    if (ctx?.AppState?.stopConnectivityScores?.stops?.[stopId]) return;
  ...
  function generateStopConnectivitySnapshot(ctx, options = {}) {
    const profile = buildProfile(options.profile);
    const params = buildParams(options.params);
    const baseStopIds = Object.keys(ctx.STOP_INFO || {}).filter((stopId) => (ctx.STOP_DEPS?.[stopId] || []).length > 0);
  ...
  function startStopConnectivityPrecompute(ctx, options = {}) {
    if (!ctx?.STOP_INFO || !ctx?.STOP_DEPS || !ctx?.AppState) return;
    if (PRECOMPUTE_STATE.running) return;
    if (ctx.AppState.stopConnectivityScores?.meta?.validation_summary) {
    ...
    ctx.AppState.stopConnectivityScores = snapshot;
